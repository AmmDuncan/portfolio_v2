...
function Navbar() {
  ...
  return (<NavbarRoot className={classes}>
...
function Sidebar(props) {
  return <SidebarRoot className={cx(["sidebar-root", { "open": props.isOpen }])}>
  ...
  return <ul className="nav-list">{
    navlist.map(({ to, display, external }) => (
      <ActiveLink key={display} href={to} activeClassName="active">
        <span className="link" onClick={() => setTimeout(props.onClose, 300)}>{display}{external &&
          <FiExternalLink />}</span>
      </ActiveLink>))}
  </ul>;
}
